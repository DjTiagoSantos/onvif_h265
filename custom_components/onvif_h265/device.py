...
class ONVIFDevice:
    ...
    async def async_setup(self) -> None:
        ...
        # Determine max resolution from profiles (H264, H265, H265+)
        self.max_resolution = max(
            profile.video.resolution.width
            for profile in self.profiles
            if profile.video.encoding in ["H264", "H265", "H265+"]
        )
    ...
    async def async_get_profiles(self) -> list[Profile]:
        ...
        for key, onvif_profile in enumerate(result):
            # Only add H264, H265, H265+ profiles
            if (
                not onvif_profile.VideoEncoderConfiguration
                or onvif_profile.VideoEncoderConfiguration.Encoding not in ["H264", "H265", "H265+"]
            ):
                continue
